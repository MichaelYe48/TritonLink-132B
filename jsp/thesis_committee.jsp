                    ...
                    try {
                        ...
                        if (action != null && action.equals("insert")) {
                            ...
                            PreparedStatement checkStmt = connection.prepareStatement(
                                "SELECT COUNT(*) FROM Thesis_Committee WHERE SSN = ? AND First_Name = ? AND Middle_Name = ? AND Last_Name = ?");
                            ...
                            if (count == 0) { // If record doesn't exist, then insert
                                PreparedStatement pstmt = connection.prepareStatement(
                                    "INSERT INTO Thesis_Committee (SSN, First_Name, Middle_Name, Last_Name) VALUES (?, ?, ?, ?)");
                        ...
                        if (action != null && action.equals("update")) {
                            PreparedStatement pstmt = connection.prepareStatement(
                                "UPDATE Thesis_Committee SET First_Name = ?, Middle_Name = ?, Last_Name = ? WHERE SSN = ?");
